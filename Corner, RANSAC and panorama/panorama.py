...
def harris_corners(img, window_size=3, k=0.04):
    ...
    mx2 = convolve(dxdx, window, mode = 'constant')
    my2 = convolve(dydy, window, mode = 'constant')
...
def simple_descriptor(patch):
    ...
    mean = patch.mean()
    std_dev = patch.std()
    std_dev = std_dev if std_dev else 1
    feature = ((patch - mean) / std_dev).reshape(-1)
    
    #the reshape(-1) basically puts everything in a 1D array
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    min = np.argmin(dists, axis=1).reshape(-1,1)
    matches = np.concatenate([np.arange(N).reshape(-1, 1), min], axis=1)
    
    matches = matches[sorted[:, 0] / sorted[:, 1] < threshold]
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for iteration in range(n_iters):
       ...
       s1, s2 = matched1[random_indices], matched2[random_indices]
       H_subset = np.linalg.lstsq(s2, s1, rcond=None)[0]
       H_subset[:, 2] = np.array([0, 0, 1])
    
       errors = np.sqrt(((matched2.dot(H_subset) - matched1) ** 2).sum(axis=-1))
       inliers_subset = errors < threshold
       n_inliers_subset = inliers_subset.sum()
    
       if n_inliers_subset > n_inliers:
          max_inliers, n_inliers = inliers_subset, n_inliers_subset
